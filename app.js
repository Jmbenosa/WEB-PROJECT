...
function fmtInput(d){return new Date(d).toISOString().slice(0,10);}
...
var members=[
  ...
  {id:'LIB-2025-003',name:'Carol White',email:'carol.w@email.com',phone:'555-0103',type:'Public',joinDate:'2025-02-01',active:1,overdue:0,totalIssued:2},
  ...
  {id:'LIB-2025-009',name:'Iris Chen',email:'iris.c@email.com',phone:'555-0109',type:'Public',joinDate:'2025-06-20',active:0,overdue:0,totalIssued:0},
  {id:'LIB-2025-010',name:'James Taylor',email:'james.t@email.com',phone:'555-0110',type:'Public',joinDate:'2025-07-05',active:1,overdue:0,totalIssued:1},
];
...
var issues=[
  ...
  {id:'IS-002',bookId:2,bookTitle:'1984',author:'George Orwell',memberId:'LIB-2025-003',memberName:'Carol White',issueDate:'2026-02-18',dueDate:'2026-03-04',status:'Active',renewals:0},
  {id:'IS-003',bookId:7,bookTitle:'A Brief History of Time',author:'Stephen Hawking',memberId:'LIB-2025-005',memberName:'Emma Davis',issueDate:'2026-02-20',dueDate:'2026-03-06',status:'Active',renewals:0},
  {id:'IS-004',bookId:10,bookTitle:'Thinking, Fast and Slow',author:'Daniel Kahneman',memberId:'LIB-2025-007',memberName:'Grace Kim',issueDate:'2026-02-22',dueDate:'2026-03-08',status:'Active',renewals:0},
  {id:'IS-005',bookId:3,bookTitle:'The Great Gatsby',author:'F. Scott Fitzgerald',memberId:'LIB-2025-002',memberName:'Bob Martinez',issueDate:'2026-01-20',dueDate:'2026-02-03',status:'Overdue',renewals:0},
  {id:'IS-006',bookId:11,bookTitle:'Sapiens',author:'Yuval Noah Harari',memberId:'LIB-2025-004',memberName:'David Lee',issueDate:'2026-01-25',dueDate:'2026-02-08',status:'Overdue',renewals:0},
  {id:'IS-007',bookId:6,bookTitle:'Clean Code',author:'Robert C. Martin',memberId:'LIB-2025-006',memberName:'Frank Wilson',issueDate:'2026-01-15',dueDate:'2026-01-29',status:'Overdue',renewals:0},
  {id:'IS-008',bookId:5,bookTitle:'The Pragmatic Programmer',author:'Andrew Hunt',memberId:'LIB-2025-006',memberName:'Frank Wilson',issueDate:'2026-01-10',dueDate:'2026-01-24',status:'Overdue',renewals:0},
  {id:'IS-009',bookId:16,bookTitle:'Educated',author:'Tara Westover',memberId:'LIB-2025-002',memberName:'Bob Martinez',issueDate:'2026-02-15',dueDate:'2026-03-01',status:'Active',renewals:0},
  {id:'IS-010',bookId:8,bookTitle:'The Catcher in the Rye',author:'J.D. Salinger',memberId:'LIB-2025-001',memberName:'Alice Johnson',issueDate:'2026-01-01',dueDate:'2026-01-15',status:'Returned',renewals:0},
  {id:'IS-011',bookId:4,bookTitle:'The C Programming Language',author:'Brian W. Kernighan',memberId:'LIB-2025-002',memberName:'Bob Martinez',issueDate:'2026-01-05',dueDate:'2026-01-19',status:'Returned',renewals:0},
  {id:'IS-012',bookId:9,bookTitle:'Brave New World',author:'Aldous Huxley',memberId:'LIB-2025-003',memberName:'Carol White',issueDate:'2026-01-10',dueDate:'2026-01-24',status:'Returned',renewals:1},
  {id:'IS-013',bookId:13,bookTitle:'The Odyssey',author:'Homer',memberId:'LIB-2025-004',memberName:'David Lee',issueDate:'2026-01-15',dueDate:'2026-01-29',status:'Returned',renewals:0},
  {id:'IS-014',bookId:15,bookTitle:'The Art of War',author:'Sun Tzu',memberId:'LIB-2025-005',memberName:'Emma Davis',issueDate:'2026-01-20',dueDate:'2026-02-03',status:'Returned',renewals:0},
  {id:'IS-015',bookId:12,bookTitle:'The Republic',author:'Plato',memberId:'LIB-2025-010',memberName:'James Taylor',issueDate:'2026-02-10',dueDate:'2026-03-10',status:'Renewed',renewals:1},
];

var currentIssueTab='active', currentReportTab='issued';
...
function initDashboard(){
  setText('dash-date', TODAY.toLocaleDateString('en-US',{weekday:'long',month:'long',day:'numeric',year:'numeric'})+' \u2014 Overview of library operations');
  updateStats();
...
function updateStats(){
  ...
  setText('mo-public',members.filter(function(m){return m.type==='Public';}).length);
...
function renderActiveIssuesList(){
  ...
  list.innerHTML=active.map(function(i){
    return '<div class="issue-item '+(i.status==='Overdue'?'overdue':'')+'">'+
      '<div><div class="issue-book">'+i.bookTitle+'</div><div class="issue-member">'+i.memberName+' &bull; Due: '+fmt(i.dueDate)+'</div></div>'+
...
function initDashCharts(){
  var cats={};
  books.forEach(function(b){cats[b.category]=(cats[b.category]||0)+b.copies;});
  var avail=books.reduce(function(s,b){return s+b.available;},0);
  var issuedB=books.reduce(function(s,b){return s+(b.copies-b.available);},0)-books.filter(function(b){return b.isLost;}).length-books.filter(function(b){return b.isDamaged;}).length;
  ...
  catChart=new Chart(document.getElementById('cat-chart'),{
    type:'bar',
    data:{labels:catLabels,datasets:[{data:catValues,backgroundColor:catLabels.map(function(){return'#2ec4a0';}),borderWidth:0,borderRadius:5}]},
    options:{plugins:{legend:{display:false}},scales:{y:{beginAtZero:true,grid:{color:'#f0f0f0'},ticks:{color:'#9bb5ad'}},x:{grid:{display:false},ticks:{color:'#9bb5ad'}}},responsive:true,maintainAspectRatio:false}
  });
  if(statusChart)statusChart.destroy();
  statusChart=new Chart(document.getElementById('status-chart'),{
    type:'doughnut',
    data:{labels:['Available','Issued','Lost','Damaged'],datasets:[{data:[avail,issuedB,lost,dmg],backgroundColor:['#2ec4a0','#3b82f6','#9ca3af','#f59e0b'],borderWidth:0,hoverOffset:4}]},
    options:{cutout:'70%',plugins:{legend:{position:'bottom',labels:{usePointStyle:true,font:{size:11}}}},responsive:true,maintainAspectRatio:false}
  });
}
...
function renderBooksTable(){
  ...
  document.getElementById('books-tbody').innerHTML=filtered.map(function(b){
    return '<tr>'+
      ...
      '<td>'+b.category+'</td><td>'+b.shelf+'</td><td>'+b.copies+'</td>'+
      '<td class="'+(b.available===0?'zero-count':'available-count')+'">'+b.available+'</td>'+
      '<td><span class="badge '+(b.isLost?'badge-lost':b.isDamaged?'badge-damaged':b.available>0?'badge-available':'badge-issued')+'">&#9679; '+(b.isLost?'Lost':b.isDamaged?'Damaged':b.available>0?'Available':'Issued')+'</span></td>'+
      '<td><div class="action-btns">'+
        (b.available>0?'<button class="btn btn-sm btn-outline" onclick="quickIssue('+b.id+')">Issue</button>':'<button class="btn btn-sm btn-outline" disabled style="opacity:.4">Issue</button>')+
...
function deleteBook(id){
  ...
  renderBooksTable();updateStats();showToast('Book deleted','success');
}
function quickIssue(id){openIssueModal(id);}
...
function renderMembersTable(){
  ...
  document.getElementById('members-tbody').innerHTML=filtered.map(function(m){
    return '<tr>'+
      ...
      '<td><span class="badge badge-'+m.type.toLowerCase()+'">'+m.type+'</span>'+(m.type==='Student'?'<div class="td-sub" style="margin-top:4px">ID: '+(m.sid||'')+' '+(m.grade||'')+' '+(m.section||'')+'</div>':'')+'</td>'+
...
function deleteMember(id){
  ...
  renderMembersTable();updateStats();showToast('Member removed','success');
}

/* ISSUES */
function switchIssueTab(tab,btn){
  currentIssueTab=tab;
  document.querySelectorAll('#issue-tabs .tab-btn').forEach(function(b){b.classList.remove('active');});
...
function renderIssuesTable(){
  ...
  document.getElementById('issues-tbody').innerHTML=filtered.map(function(i){
    ...
    var daysHtml='';
    if(i.status==='Returned') daysHtml='<span class="text-muted">&mdash;</span>';
    else if(diff<0)  daysHtml='<span class="days-left-over">'+Math.abs(diff)+'d over</span>';
    else if(diff===0)daysHtml='<span class="days-left-warn">Due today</span>';
    else if(diff<=3) daysHtml='<span class="days-left-warn">'+diff+'d left</span>';
    else             daysHtml='<span class="days-left-ok">'+diff+'d left</span>';
...
function renewIssue(id){
  ...
  issue.renewals++;
  var due=new Date(issue.dueDate);due.setDate(due.getDate()+14);
  issue.dueDate=fmtInput(due);issue.status='Renewed';
  renderIssuesTable();updateStats();showToast('Book renewed for 14 more days!','success');
}
function quickReturn(id){
  ...
  if(book){book.available++;if(book.available>0)book.status='Available';}
  var member=members.find(function(m){return m.id===issue.memberId;});
  if(member){member.active=Math.max(0,member.active-1);if(wasOverdue)member.overdue=Math.max(0,member.overdue-1);}
  renderIssuesTable();renderActiveIssuesList();updateStats();showToast('Book returned successfully!','success');
}

/* REPORTS */
function switchReportTab(tab,btn){
  currentReportTab=tab;
  document.querySelectorAll('.report-panel .tab-btn').forEach(function(b){b.classList.remove('active');});
  btn.classList.add('active');renderReportTable();
}
function renderReportTable(){
  ...
  if(currentReportTab==='damaged') filtered=filtered.filter(function(i){var b=books.find(function(b){return b.id===i.bookId;});return b&&(b.isLost||b.isDamaged);});
  if(q) filtered=filtered.filter(function(i){return i.bookTitle.toLowerCase().includes(q)||i.memberName.toLowerCase().includes(q)||i.id.toLowerCase().includes(q);});
  document.getElementById('report-tbody').innerHTML=filtered.map(function(i){
    return '<tr>'+
      '<td><span class="link">'+i.id+'</span></td>'+
      '<td>'+i.bookTitle+'</td><td>'+i.memberName+'</td>'+
      '<td>'+fmt(i.issueDate)+'</td>'+
      '<td class="'+(i.status==='Overdue'?'due-overdue':'')+'">'+fmt(i.dueDate)+'</td>'+
      '<td>'+i.renewals+'</td>'+
      '<td>'+statusBadge(i.status,i.renewals)+'</td>'+
...
function initReportCharts(){
  var catCount={};
  issues.forEach(function(i){var b=books.find(function(b){return b.id===i.bookId;});if(b)catCount[b.category]=(catCount[b.category]||0)+1;});
  var rptLabels=Object.keys(catCount);var rptValues=Object.values(catCount);
  if(rptCatChart)rptCatChart.destroy();
  rptCatChart=new Chart(document.getElementById('rpt-cat-chart'),{
    type:'bar',
    data:{labels:rptLabels,datasets:[{data:rptValues,backgroundColor:rptLabels.map(function(){return'#2ec4a0';}),borderWidth:0,borderRadius:5}]},
    options:{plugins:{legend:{display:false}},scales:{y:{beginAtZero:true,grid:{color:'#f0f0f0'},ticks:{color:'#9bb5ad'}},x:{grid:{display:false},ticks:{color:'#9bb5ad'}}},responsive:true,maintainAspectRatio:false}
  });
  var monthData=[0,0,0,0,0,0];
  var monthMap={9:0,10:1,11:2,0:3,1:4,2:5};
  issues.forEach(function(i){var m=new Date(i.issueDate).getMonth();if(monthMap[m]!==undefined)monthData[monthMap[m]]++;});
  if(rptMonthlyChart)rptMonthlyChart.destroy();
  rptMonthlyChart=new Chart(document.getElementById('rpt-monthly-chart'),{
    type:'line',
    data:{labels:['Oct','Nov','Dec','Jan','Feb','Mar'],datasets:[{data:monthData,borderColor:'#2ec4a0',backgroundColor:'rgba(46,196,160,0.08)',tension:.4,pointBackgroundColor:'#2ec4a0',pointRadius:5,fill:true}]},
    options:{plugins:{legend:{display:false}},scales:{y:{beginAtZero:true,grid:{color:'#f0f0f0'}},x:{grid:{display:false}}},responsive:true,maintainAspectRatio:false}
  });
}
function exportCSV(){
  var rows=[['Issue ID','Book Title','Member','Issue Date','Due Date','Renewals','Status']];
  issues.forEach(function(i){rows.push([i.id,'"'+i.bookTitle+'"','"'+i.memberName+'"',i.issueDate,i.dueDate,i.renewals,i.status]);});
  var csv=rows.map(function(r){return r.join(',');}).join('\n');
  var a=document.createElement('a');a.href='data:text/csv;charset=utf-8,'+encodeURIComponent(csv);a.download='library-report.csv';a.click();
...
function openRegisterModal(){document.getElementById('m-join').value=fmtInput(TODAY);openModal('register-member-modal');}
...
function openIssueModal(bookId,memberId){
  ...
  sel.innerHTML=books.filter(function(b){return b.available>0;}).map(function(b){
  ...
  msel.innerHTML=members.map(function(m){
    return '<option value="'+m.id+'"'+(m.id===memberId?' selected':'')+'>'+m.name+' ('+m.id+')</option>';
  }).join('');
...
function openReturnModal(){
  ...
  sel.innerHTML=active.map(function(i){return '<option value="'+i.id+'">'+i.id+' \u2014 '+i.bookTitle+' ('+i.memberName+')</option>';}).join('');
...
function addBook(){
  ...
  books.push({id:books.length+1,isbn:isbn,title:title,author:author,
  ...
  ['b-isbn','b-shelf','b-title','b-author','b-desc'].forEach(function(id){var el=document.getElementById(id);if(el)el.value='';});
  document.getElementById('b-copies').value='1';document.getElementById('b-avail').value='1';
  renderBooksTable();updateStats();showToast('Book added successfully!','success');
}
function registerMember(){
  ...
  members.push({id:newId,name:name,email:email,
    ...
    active:0,overdue:0,totalIssued:0});
  closeModal('register-member-modal');
  ['m-name','m-email','m-phone','m-address','m-sid','m-grade','m-section'].forEach(function(id){var el=document.getElementById(id);if(el)el.value='';});
  renderMembersTable();updateStats();showToast('Member registered!','success');
}
function issueBook(){
  ...
  issues.push({id:'IS-'+String(issues.length+1).padStart(3,'0'),
    bookId:bookId,bookTitle:book.title,author:book.author,
    memberId:memberId,memberName:member?member.name:memberId,
    issueDate:document.getElementById('i-date').value,
    dueDate:document.getElementById('i-due').value,
    status:'Active',renewals:0});
  book.available--;if(book.available===0)book.status='Issued';
  if(member){member.active++;member.totalIssued++;}
  closeModal('issue-modal');
  renderActiveIssuesList();updateStats();
...
function returnBook(){
  ...
  quickReturn(id);closeModal('return-modal');
}
